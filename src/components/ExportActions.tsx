...
function buildExportHTML(
  ...
  const rows = DAYS.map(day => {
    const cells = PERIODS.map(period => {
      ...
      if (classes.length === 0) return `<td style="padding:6px; border:1px solid #e2e8f0; background:#fafafa;"></td>`;
  ...
  DAYS.forEach(day => PERIODS.forEach(p => (timetable[day][p] || []).forEach(cls => subjects.add(cls.subject))));
  const legendItems = Array.from(subjects).map(sub => {
    const col = getSubjectColor(sub);
    return `<div style="display:inline-flex;align-items:center;gap:6px;margin:4px 8px 4px 0;">
      <div style="width:12px;height:12px;background:${col.border};border-radius:3px;"></div>
      <span style="font-size:10px;color:#1e293b;font-weight:600;">${sub}</span>
    </div>`;
  }).join('');

  return `
    <div style="font-family:'Segoe UI',Arial,sans-serif;background:#ffffff;padding:28px;width:1600px;min-height:600px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;">
        <div>
          <h1 style="font-size:22px;font-weight:800;color:#051F20;margin:0 0 4px 0;letter-spacing:-0.03em;">5TH SEM TIMETABLE</h1>
          <p style="font-size:12px;color:#235E47;margin:0;">
        ...
        <div style="text-align:right;">
          <p style="font-size:10px;color:#64748b;margin:0;">Generated by 5th Sem Timetable Planner</p>
          <p style="font-size:10px;color:#64748b;margin:4px 0 0 0;">Made by Debayan Samal</p>
        </div>
      </div>
      <div style="overflow:visible;margin-bottom:16px;">
        <table style="border-collapse:collapse;width:100%;table-layout:fixed;">
          <thead>
            <tr>
              <th style="padding:10px 14px;background:#051F20;color:#DAF1DE;font-size:11px;font-weight:700;border:1px solid #163E32;width:100px;min-width:100px;">DAY</th>
              ${headerCells}
            </tr>
          </thead>
          <tbody>${rows}</tbody>
      ...
      <div style="border-top:2px solid #e2e8f0;padding-top:12px;">
        <p style="font-size:10px;font-weight:700;color:#051F20;margin:0 0 6px 0;">SUBJECT LEGEND</p>
        <div>${legendItems}</div>
      </div>
    </div>`;
}
